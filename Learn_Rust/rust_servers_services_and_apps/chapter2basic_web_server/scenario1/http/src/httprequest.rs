...
impl From<String> for HttpRequest {
    fn from(req: String) -> Self {
        ...
        let mut parsed_resources = Resource::Path("".to_string());
        let mut parsed_headers = HashMap::new();
        let mut parsed_msg_body = "";

        //Read each line in the incoming HTTP request
        for line in req.lines(){
            //if the line read is request line, call funtion process_req_line()
        ...
        HttpRequest { method: parsed_method, version: parsed_version, resource: parsed_resources, headers: parsed_headers, msg_body: parsed_msg_body.to_string() }
    }
